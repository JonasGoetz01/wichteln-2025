  ...
  try {
    ...
    const recentActivity = participants
      ...
      .map((p) => ({
        id: p.id,
        userName:
          `${p.user.firstName || "User"} ${p.user.lastName || ""}`.trim(),
    ...
    const classDistribution = participantsByClass.filter(
      (cls) => cls.count > 0,
    );
    ...
    const stats = {
      ...
      averageParticipantsPerClass:
        totalClasses > 0 ? Math.round(totalParticipants / totalClasses) : 0,
    };

    // Growth metrics
    const lastWeekRegistrations = participants.filter(
      (p) => p.createdAt >= subDays(new Date(), 7),
    ).length;

    const previousWeekRegistrations = participants.filter(
      (p) =>
